...
const [personToDelete, setPersonToDelete] = useState<Person | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (editing) {
    setPersons((prev) =>prev.map((person) => person.id === formData.id ? { ...formData } : person));
    } else {
...
const handleDelete = () => {
  ...
  const columns: ColumnDef<Person>[] = [
    ...
    { accessorKey:"vehicles", header:"Vehicles Linked", enableSorting:true },
    { accessorKey:"trees", header:"Trees Assigned", enableSorting:true },
    {
    accessorKey:"status",
    header:"Status",
    cell: ({ row }) => (
...
>
  ...
  <>
    ...
    <PersonModal isOpen={showModal} onClose={()=> setShowModal(false)}
      editing={editing}
      formData={formData}
